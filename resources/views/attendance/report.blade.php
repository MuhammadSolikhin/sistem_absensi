...
@section('content')
    <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center">
            ...
            <div>
                <a href="{{ route('laporan.index', ['start_date' => now()->startOfWeek()->toDateString(), 'end_date' => now()->endOfWeek()->toDateString()]) }}"
                    class="btn btn-sm btn-outline-primary">Minggu Ini</a>
                <a href="{{ route('laporan.index', ['start_date' => now()->startOfMonth()->toDateString(), 'end_date' => now()->endOfMonth()->toDateString()]) }}"
                    class="btn btn-sm btn-outline-primary">Bulan Ini</a>
            </div>
        </div>
                    ...
                    <tbody>
                        @forelse($attendances as $log)
                            <tr>
                                ...
                                <td>
                                    @if($log->capture_image_path)
                                        <a href="{{ asset('storage/' . $log->capture_image_path) }}" target="_blank">
                                            <img src="{{ asset('storage/' . $log->capture_image_path) }}" class="rounded-circle"
                                ...
                                <td>
                                    <span class="badge bg-label-success">{{ $log->status_kehadiran ?? 'Hadir' }}</span>
                                </td>
                                <td>{{ $log->group->nama_group ?? '-' }}</td>
                                <td>
                                    @php
                                ...
                                @if(auth()->user()->role == 'admin')
                                    <td>
                                        <form action="{{ route('attendance.destroy', $log->id) }}" method="POST"
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="text-center py-5">
                                    <img src="{{ asset('assets/img/illustrations/girl-doing-yoga-light.png') }}" width="150"
                                        class="mb-3">

                    ...
                    <tr>
                        ...
                        <td>
                            @if($item->datasets_count >= 10)
                                <span class="badge bg-success"><i class="bx bx-check-double me-1"></i> Siap ({{ $item->datasets_count }} Foto)</span>
                            @elseif($item->datasets_count > 0)
                                <span class="badge bg-warning"><i class="bx bx-error me-1"></i> Kurang ({{ $item->datasets_count }}/10)</span>
                            @else
                        ...
                        <td>
                            <div class="dropdown">
                                ...
                                <div class="dropdown-menu">
                                    <a class="dropdown-item" href="{{ route('jamaah.edit', $item->id) }}">
                                        <i class="bx bx-edit-alt me-1"></i> Edit & Upload Foto
                                    </a>
                                    <form action="{{ route('jamaah.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus data ini? Semua dataset wajah akan ikut terhapus.')">
                                        @csrf
                                        @method('DELETE')
